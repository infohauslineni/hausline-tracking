import { AlertTriangle, ArrowLeft, Ban, Boxes, CalendarDays, Check, CheckCircle2, Clipboard, FileText, ImageUp, MessageCircle, Package, PackageCheck, Pencil, Share2, Ticket, Truck, UserRound, Wallet, X, Zap } from 'lucide-react'
...
function facturaItemsDePedido(pedido: Pedido): FacturaData['items'] {
  ...
  // El total del pedido ya viene NETO del cupón/descuento: se suma de vuelta para que la
  // diferencia (envío rápido) no se confunda, y el descuento sale como su propia línea.
...
export function PedidoDetailPage() {
  ...
  // Abre la pantalla de cobro (subir comprobante + elegir cuenta) para un destino dado.
  // 'pagado' registra el pago y congela la bodega SIN entregar todavía (para cuando el cliente
  // ya pagó pero la entrega es después); 'entregado' cobra al momento de entregar. Suma al
  // saldo el cargo por bodega acumulado.
  const iniciarCobro = (destino: 'pagado' | 'entregado') => {
  ...
  return <div>
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[1.5fr_.75fr]">
      <div className="min-w-0 space-y-5">
        ...
        <PedidoArchivos pedidoId={pedido.id} codigo={pedido.codigo} estadoPedido={pedido.estado} items={pedido.pedido_items ?? []} qcGeneralEnviadoAt={pedido.qc_general_enviado_at} onQualityReady={setQualityPhotosReady} onEstadoAvanzado={(updated) => setPedido((current) => current ? { ...current, ...updated } : updated)} />
      ...
      <aside className="min-w-0 space-y-5"><section className="form-section"><h2 className="flex items-center gap-2 font-semibold"><UserRound size={18} className="text-accent" /> Cliente</h2><div className="mt-4"><strong>{pedido.clientes?.nombre}</strong><p className="mt-1 text-sm text-muted">{pedido.clientes?.whatsapp}</p></div></section>{pedido.entrega_solicitada_at && pedido.entrega_direccion && <section className="form-section"><h2 className="flex items-center gap-2 font-semibold"><Truck size={18} className="text-accent" /> Envío solicitado por el cliente</h2><p className="mt-1 text-xs text-muted">{new Intl.DateTimeFormat('es-NI', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(pedido.entrega_solicitada_at))}</p><div className="mt-3 text-sm"><strong>{pedido.entrega_direccion.nombre}</strong><p className="mt-1 text-muted">{pedido.entrega_direccion.direccion}{pedido.entrega_direccion.referencia ? ` · ${pedido.entrega_direccion.referencia}` : ''}</p><p className="text-muted">{[pedido.entrega_direccion.ciudad, pedido.entrega_direccion.departamento, pedido.entrega_direccion.pais].filter(Boolean).join(', ')}{pedido.entrega_direccion.codigo_postal ? ` · CP ${pedido.entrega_direccion.codigo_postal}` : ''}</p><p className="mt-2">Delivery: <strong>{pedido.entrega_costo != null ? `US${Number(pedido.entrega_costo).toFixed(2)}` : 'a cotizar'}</strong></p>{pedido.entrega_direccion.lat != null && pedido.entrega_direccion.lng != null && <a className="subtle-button mt-3 w-full" href={`https://www.google.com/maps?q=${pedido.entrega_direccion.lat},${pedido.entrega_direccion.lng}`} target="_blank" rel="noopener noreferrer">Ver ubicación en Google Maps</a>}</div></section>}{esAdmin && <><section className="form-section"><h2 className="font-semibold">Resumen de pago</h2><div className="mt-4 space-y-3 text-sm">{Number(pedido.descuento || 0) > 0 && <div className="flex items-center justify-between gap-3 rounded-lg border border-accent/30 bg-accent/[.06] px-3 py-2"><span className="flex items-center gap-2 text-muted"><Ticket size={14} className="text-accent" /> {pedido.cupon_codigo ? <>Cupón <strong className="font-mono text-white">{pedido.cupon_codigo}</strong></> : 'Descuento'}</span><strong className="font-mono text-accent">−${Number(pedido.descuento).toFixed(2)}</strong></div>}<PayRow label="Total" value={pedido.total} /><PayRow label="Abono" value={pedido.abono} /><PayRow label="Costo real" value={costoReal} />{envioCliente > 0 && <PayRow label="Envío del cliente (no es ganancia)" value={envioCliente} />}<PayRow label="Ganancia estimada" value={gananciaEstimada} /><div className="border-t border-line pt-3"><PayRow label="Saldo pendiente" value={pedido.saldo} accent /></div></div>{Number(pedido.saldo || 0) > 0.01 && pedido.estado !== 'entregado' && pedido.estado !== 'cancelado' && <button className="primary-button mt-4 w-full" onClick={() => iniciarCobro('pagado')}><Wallet size={16} /> Registrar pago (subir comprobante)</button>}{pedido.estado !== 'entregado' && pedido.estado !== 'cancelado' && <button className="subtle-button mt-2 w-full" onClick={() => { const actual = pedido.pedido_items?.find((it) => it.producto === 'Envío / delivery'); setEnvioMonto(actual ? String(actual.precio_unitario) : ''); setEnvioNota(actual?.talla ?? ''); setEnvioOpen(true) }}><Truck size={16} /> {pedido.pedido_items?.some((it) => it.producto === 'Envío / delivery') ? 'Editar envío' : 'Agregar envío'}</button>}<button className="subtle-button mt-2 w-full" onClick={() => abrirFactura()}><FileText size={16} /> Descargar factura</button>{Number(pedido.saldo || 0) > 0.01 && <button className="subtle-button mt-2 w-full" onClick={() => abrirFactura(true)}><FileText size={16} /> Comprobante pagado (imprimir antes)</button>}</section><section className="form-section"><h2 className="font-semibold">Gastos asociados</h2><div className="mt-4 space-y-3">{pedido.gastos?.length ? pedido.gastos.map((gasto) => <div className="flex justify-between gap-3 text-xs" key={gasto.id}><span className="text-muted">{gasto.categoria}</span><strong>${Number(gasto.monto).toFixed(2)}</strong></div>) : <p className="text-xs text-muted">Todavía no hay gastos registrados para este pedido.</p>}</div>{pedido.estado !== 'cancelado' && <button className="subtle-button mt-4 w-full" onClick={() => setGastoOpen(true)}><Truck size={16} /> Agregar costo de envío</button>}</section></>}</aside>
    </div>
