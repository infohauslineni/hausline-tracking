import { ArrowUpRight, Boxes, CalendarDays, CheckCircle2, Download, Package, Plus, Search, Trash2, XCircle } from 'lucide-react'
...
export function PedidosPage() {
  ...
  return <div>
    ...
    <section className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4"><MiniMetric label="Total" value={pedidos.length} icon={Boxes} /><MiniMetric label="Activos" value={pedidos.filter((p) => !['entregado','cancelado'].includes(p.estado)).length} icon={CalendarDays} tone="blue" /><MiniMetric label="Completados" value={pedidos.filter((p) => p.estado === 'entregado').length} icon={CheckCircle2} tone="emerald" /><MiniMetric label="Cancelados" value={pedidos.filter((p) => p.estado === 'cancelado').length} icon={XCircle} tone="danger" /></section>
