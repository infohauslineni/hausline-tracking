import { ArrowUpRight, Boxes, CalendarDays, CircleDollarSign, Plus, Search, Trash2 } from 'lucide-react'
...
function OrderCard({ pedido, onDelete }: { pedido: Pedido; onDelete: () => void }) { return <article className="rounded-2xl border border-line bg-panel p-4"><div className="flex items-start justify-between gap-3"><Link to={`/pedidos/${pedido.id}`} className="min-w-0 flex-1"><strong className="text-sm tracking-wide">{pedido.codigo}</strong><p className="mt-1 text-xs text-muted">{pedido.clientes?.nombre}</p></Link><div className="flex gap-1.5"><Link to={`/pedidos/${pedido.id}`} className="table-action table-action-open" aria-label="Abrir pedido"><ArrowUpRight size={17} /></Link><button className="table-action table-action-danger" onClick={onDelete} aria-label="Eliminar pedido"><Trash2 size={17} /></button></div></div><Link to={`/pedidos/${pedido.id}`} className="block"><div className="mt-4"><Status estado={pedido.estado} /></div><p className="mt-4 truncate text-xs text-muted">{pedido.pedido_items?.map((item) => item.producto).join(', ')}</p><div className="mt-3 flex items-end justify-between border-t border-line pt-3"><div><span className="block text-[10px] uppercase tracking-wider text-muted">Total</span><strong>${Number(pedido.total).toFixed(2)}</strong></div><div className="text-right"><span className="block text-[10px] uppercase tracking-wider text-muted">Saldo</span><strong className={pedido.saldo > 0 ? 'text-amber-300' : 'text-[#62eaa0]'}>${Number(pedido.saldo).toFixed(2)}</strong></div></div></Link></article> }
