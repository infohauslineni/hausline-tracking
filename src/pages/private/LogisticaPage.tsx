...
export function LogisticaPage() {
  ...
  // cron diario) y recarga los trayectos. Ideal para ir viendo la ubicación en vivo.
  // Manda la sesión del admin para que el endpoint autorice.
  const actualizarSeguimiento = async () => {
    if (!supabase) return
    setActualizando(true)
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) { toast.error('Vuelve a iniciar sesión para actualizar.'); return }
      const res = await fetch('/api/track17-poll', { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}` } })
      const json = await res.json().catch(() => ({}))
      if (!res.ok) { toast.error('No se pudo actualizar el seguimiento.'); return }
      const routes = await listarTrayectos()
      setTrayectos(routes)
      toast.success(Number(json?.avanzados) > 0 ? 'Seguimiento actualizado · hay avances de estado.' : 'Seguimiento actualizado.')
    } catch {
  ...
  // Refresco automático de los trackings: 17track avisa por su cuenta, así que
  // mientras esta página esté abierta la barra de progreso se actualiza sola
  // (cada 2 min, en silencio) sin que tengas que recargar ni revisar cliente por cliente.
  useEffect(() => {
    if (!isSupabaseConfigured) return
    const timer = window.setInterval(() => { void listarTrayectos().then(setTrayectos).catch(() => {}) }, 120_000)
  ...
  const deliverRoute = async (route: Trayecto) => { if (!window.confirm('¿Confirmas que este paquete ya fue recibido en la bodega de USA (Miami)?')) return; try { if (isSupabaseConfigured) await marcarTrayectoEntregado(route.id); setTrayectos((current) => current.map((item) => item.id === route.id ? { ...item, estado: 'entregado', activo: false, fecha_entrega: new Date().toISOString(), updated_at: new Date().toISOString() } : item)); toast.success('Marcado como recibido en USA y movido al apartado inferior.') } catch { toast.error('No se pudo actualizar el tracking.') } }
  ...
  const reopenRoute = async (route: Trayecto) => { if (!window.confirm('¿Deshacer “Recibido en USA” y devolver este tracking a los activos?')) return; try { if (isSupabaseConfigured) await reabrirTrayecto(route.id); setTrayectos((current) => current.map((item) => item.id === route.id ? { ...item, estado: 'en_transito', activo: true, fecha_entrega: null, updated_at: new Date().toISOString() } : item)); toast.success('El tracking volvió a activos.') } catch { toast.error('No se pudo corregir el tracking.') } }
