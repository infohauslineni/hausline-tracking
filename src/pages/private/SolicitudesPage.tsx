...
export function SolicitudesPage() {
  ...
  const confirmar = async (grupo: Solicitud[], abono: number, comprobante: File | null, destino: DestinoPago, descuento: number) => {
    ...
    try {
      ...
      // El error de Supabase es un objeto (PostgrestError), no un Error: leemos message/code.
      const e = (error ?? {}) as { message?: unknown; code?: unknown }
      const msg = typeof e.message === 'string' && e.message ? e.message : 'No se pudo confirmar el encargo.'
      const faltaFuncion = e.code === 'PGRST202' || /function|does not exist|schema cache|no such function/i.test(msg)
      toast.error(faltaFuncion ? 'Falta aplicar la migración del descuento (202609070002) en Supabase, luego reintentá.' : msg)
