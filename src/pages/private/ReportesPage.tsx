...
import { guardarConfiguracionFinanzas, obtenerConfiguracionFinanzas, obtenerGananciaRealizada, periodoComercial, retirarGanancia } from '../../services/finanzas.service'
...
export function ReportesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const result = await obtenerGananciaRealizada(period.desde, period.hasta)
      setConfig(currentConfig)
      setData(result)
      const reserve = Math.max(0, result.ganancia_disponible * currentConfig.porcentaje_reserva_negocio / 100)
      const available = Math.max(0, result.ganancia_disponible - reserve)
  ...
  const withdrawable = Math.max(0, data.ganancia_disponible - reserve)
  ...
  return <div>
    ...
    <section className="mt-6 rounded-2xl border border-accent/25 bg-accent/[.055] p-5 sm:p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-accent"><CalendarDays size={16} /> Periodo comercial</p><h2 className="mt-2 text-xl font-semibold">{formatDate(data.desde)} — {formatDate(data.hasta)}</h2><p className="mt-2 text-sm text-muted">{isWithdrawalDay ? <b className="text-accent">Hoy es tu día de retiro.</b> : <>Próximo retiro recomendado: <b className="text-white">{withdrawalDate}</b></>}</p></div><div className="rounded-2xl border border-accent/20 bg-black/20 p-5 text-right"><span className="text-xs text-muted">Puedes retirar sin usar la reserva</span><strong className="mt-1 block text-3xl text-accent">USD {withdrawable.toFixed(2)}</strong></div></div></section>
