import { Boxes, CircleDollarSign, Eye, ImagePlus, PackageCheck, Pencil, Plus, Search, ShoppingBag, Tag, Trash2, X } from 'lucide-react'
...
const empty = { fecha: new Date().toISOString().slice(0, 10), producto_id: '', codigo: '', producto: '', marca: '', talla_color: '', cantidad: '1', costo_unitario: '', gastos_adicionales: '', precio_venta_estimado: '', metodo: 'Transferencia', notas: 'Producto para entrega inmediata', inversionYaRegistrada: false }
const statusLabel: Record<Inversion['estado'], string> = { en_inventario: 'Disponible', reservado: 'Reservado', vendido: 'Vendido', descartado: 'Retirado' }
...
// Lo disponible se muestra primero; lo vendido o retirado se va al fondo.
const statusOrder: Record<Inversion['estado'], number> = { en_inventario: 0, reservado: 1, vendido: 2, descartado: 3 }
const linkedExpenses = (item: Inversion) => (item.gastos ?? []).reduce((sum, expense) => sum + Number(expense.monto), 0)
const totalStockCost = (item: Inversion) => Number(item.costo_unitario) * Number(item.cantidad) + Number(item.gastos_adicionales) + linkedExpenses(item)
...
export function StockPage() {
  ...
  const load = () => void Promise.all([listarInversiones(), listarProductos()]).then(([investments, catalog]) => { setItems(investments); setProducts(catalog.filter((item) => item.activo)) }).catch(() => toast.error('No se pudo cargar el stock.'))
  useEffect(load, [])
  const available = items.filter((item) => item.estado === 'en_inventario' || item.estado === 'reservado')
  const totals = useMemo(() => available.reduce((result, item) => { result.units += item.cantidad; result.cost += totalStockCost(item); result.sale += Number(item.precio_venta_estimado) * item.cantidad; return result }, { units: 0, cost: 0, sale: 0 }), [available])
  ...
  const visible = useMemo(() => {
    ...
    return items
      .filter((item) => (filter === 'todos' || item.estado === filter) && (!term || [item.producto, item.codigo, item.marca, item.talla_color].some((v) => v?.toLowerCase().includes(term))))
      .sort((a, b) => statusOrder[a.estado] - statusOrder[b.estado])
  }, [items, search, filter])
  const updateStatus = async (item: Inversion, estado: Inversion['estado']) => { try { const updated = await cambiarEstadoInversion(item.id, estado); setItems((all) => all.map((current) => current.id === updated.id ? updated : current)); toast.success('Stock actualizado.') } catch { toast.error('No se pudo actualizar.') } }
  const remove = async (item: Inversion) => {
    if (!window.confirm(`¿Eliminar "${item.producto}" del stock? Se devolverá su capital invertido a Mi cuenta. Esta acción no se puede deshacer.`)) return
    try { await eliminarInversion(item.id); setItems((all) => all.filter((current) => current.id !== item.id)); toast.success('Producto eliminado del stock.') } catch { toast.error('No se pudo eliminar el producto.') }
  }

  return <div><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Entrega inmediata</p><h1 className="page-title">Stock inmediato</h1><p className="page-subtitle">Productos disponibles para vender y entregar sin esperar importación.</p></div><button className="primary-button px-5" onClick={() => { setEditing(null); setOpen(true) }}><Plus size={17} /> Agregar al stock</button></div>
    <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={Boxes} label="Unidades disponibles" value={String(totals.units)} /><Metric icon={CircleDollarSign} label="Capital en stock" value={`USD ${totals.cost.toFixed(2)}`} /><Metric icon={ShoppingBag} label="Venta potencial" value={`USD ${totals.sale.toFixed(2)}`} /><Metric icon={PackageCheck} label="Ganancia potencial" value={`USD ${Math.max(0, totals.sale - totals.cost).toFixed(2)}`} accent /></div>
    <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_240px]"><div className="relative"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" size={18} /><input className="search-input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Producto, código o marca" /></div><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="todos">Todos los estados ({items.length})</option>{(Object.keys(statusLabel) as Inversion['estado'][]).map((estado) => <option value={estado} key={estado}>{statusLabel[estado]} ({counts[estado] ?? 0})</option>)}</select></div>
    <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map((item) => { const cerrado = item.estado === 'vendido' || item.estado === 'descartado'; return <article className={`panel-card transition ${cerrado ? 'opacity-70' : ''}`} key={item.id}><div className="flex items-start justify-between gap-3"><button className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-xl bg-accent/10 text-accent" onClick={() => setPreview(item)} aria-label={`Vista para cliente de ${item.producto}`}>{item.imagen ? <img src={item.imagen} alt={item.producto} className="size-full object-cover" /> : <PackageCheck size={22} />}</button><div className="flex items-start gap-2"><select className="w-auto text-xs" value={item.estado} onChange={(event) => void updateStatus(item, event.target.value as Inversion['estado'])}>{Object.entries(statusLabel).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><button className="table-action" onClick={() => setPreview(item)} aria-label={`Vista para cliente de ${item.producto}`}><Eye size={16} /></button><button className="table-action table-action-edit" onClick={() => { setEditing(item); setOpen(true) }} aria-label={`Editar ${item.producto}`}><Pencil size={16} /></button><button className="table-action table-action-danger" onClick={() => void remove(item)} aria-label={`Eliminar ${item.producto}`}><Trash2 size={16} /></button></div></div><div className="mt-4 flex items-center justify-between gap-2"><p className="text-[10px] font-semibold uppercase tracking-wider text-accent">{item.codigo || 'SIN CÓDIGO'}</p><span className={`rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${statusTone[item.estado]}`}>{statusLabel[item.estado]}</span></div><strong className="mt-1 block text-lg">{item.producto}</strong><p className="mt-1 text-xs text-muted">{[item.marca, item.talla_color, `${item.cantidad} unidad${item.cantidad === 1 ? '' : 'es'}`].filter(Boolean).join(' · ')}</p><div className="mt-5 grid grid-cols-3 gap-3 border-t border-line pt-4"><Money label="Costo total" value={totalStockCost(item)} /><Money label="Venta posible" value={Number(item.precio_venta_estimado) * Number(item.cantidad)} accent /><Money label="Ganancia" value={Math.max(0, Number(item.precio_venta_estimado) * Number(item.cantidad) - totalStockCost(item))} green /></div>{linkedExpenses(item) > 0 && <p className="mt-3 text-[11px] text-muted">Incluye USD {linkedExpenses(item).toFixed(2)} en gastos asociados.</p>}{(item.estado === 'en_inventario' || item.estado === 'reservado') && <button className="primary-button mt-4 w-full justify-center" onClick={() => setSelling(item)}><Tag size={15} /> Vender ahora</button>}{item.estado === 'vendido' && <p className="mt-4 rounded-xl border border-green-400/25 bg-green-400/[.07] p-2.5 text-center text-xs font-semibold text-green-300">Vendido</p>}</article> })}</div>
    {!items.length && <div className="mt-6 rounded-2xl border border-dashed border-line p-10 text-center text-sm text-muted">Agrega aquí los productos que tengas disponibles para entrega inmediata.</div>}
    ...
    <StockModal open={open} item={editing} products={products} onClose={() => { setOpen(false); setEditing(null) }} onSaved={(item) => { setItems((all) => editing ? all.map((current) => current.id === item.id ? item : current) : [item, ...all]); setOpen(false); setEditing(null) }} />
...
function SellModal({ item, onClose, onSold }: { item: Inversion | null; onClose: () => void; onSold: (item: Inversion) => void }) {
  ...
  if (!item) return null
  const precio = Number(form.precio_venta), recibido = Number(form.monto_recibido), costo = totalStockCost(item)
  ...
  return <Modal open={Boolean(item)} onClose={onClose} title={`Vender · ${item.producto}`} description="Registra la venta directa de stock. No crea un pedido de importación.">
...
function StockModal({ open, item, products, onClose, onSaved }: { open: boolean; item: Inversion | null; products: Producto[]; onClose: () => void; onSaved: (item: Inversion) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      onSaved(saved)
      toast.success(item ? 'Producto actualizado.' : 'Producto agregado al stock inmediato.')
  ...
  return <Modal open={open} onClose={onClose} title={item ? 'Editar producto de entrega inmediata' : 'Agregar producto de entrega inmediata'}><form className="form-grid" onSubmit={(event) => void submit(event)}>
    ...
    {!item && <label className="col-span-full flex items-center gap-3 rounded-xl border border-line p-4 text-sm"><input type="checkbox" className="size-5 accent-[#b7ff00]" checked={form.inversionYaRegistrada} onChange={(event) => setForm({ ...form, inversionYaRegistrada: event.target.checked })} /><span><strong>¿Esta inversión ya estaba registrada?</strong><small className="mt-1 block text-muted">Márcalo para no descontarla nuevamente de Mi cuenta.</small></span></label>}
    <div className="col-span-full rounded-xl border border-accent/20 bg-accent/[.05] p-4 text-sm">{item ? 'Se actualizará este producto sin registrar una inversión nueva.' : <>Se registrará una inversión de <strong className="text-accent">USD {total.toFixed(2)}</strong>{form.inversionYaRegistrada ? ' sin descontarla nuevamente de Mi cuenta.' : ' y se descontará de Mi cuenta.'}</>}</div>
    <div className="col-span-full flex justify-end gap-2"><button type="button" className="subtle-button" onClick={onClose}>Cancelar</button><button className="primary-button px-5" disabled={saving}>{saving ? 'Guardando…' : item ? 'Guardar cambios' : 'Agregar al stock'}</button></div>
  </form></Modal>
}
