...
import { CuponModal } from './CuponesPage'
...
export function ClientesPage() {
  ...
  return <div>
    ...
    {loading ? <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{[1,2,3].map((n) => <div key={n} className="h-44 animate-pulse rounded-2xl border border-line bg-panel" />)}</div> : filtered.length === 0 ? <EmptyClients onAdd={openNew} /> : <>
      <div className="mt-5 grid gap-3 md:hidden">{filtered.map((client) => <ClientCard key={client.id} client={client} onEdit={() => openEdit(client)} onDelete={() => void remove(client)} onCupon={() => setCuponFor(client)} />)}</div>
      <div className="mt-5 hidden overflow-hidden rounded-2xl border border-line bg-panel md:block"><table className="data-table"><thead><tr><th>Cliente</th><th>WhatsApp</th><th>Ciudad</th><th>Creado</th><th aria-label="Acciones" /></tr></thead><tbody>{filtered.map((client) => <tr key={client.id}><td><Link to={`/clientes/${client.id}`} className="transition hover:text-accent"><strong>{client.nombre}</strong></Link><span>{client.notas || 'Sin notas internas'}</span></td><td><span>{client.whatsapp}</span></td><td><span>{[client.ciudad, client.departamento].filter(Boolean).join(', ') || 'Sin ubicación'}</span></td><td><span>{new Intl.DateTimeFormat('es-NI').format(new Date(client.created_at))}</span></td><td><div className="flex justify-end gap-1.5"><a className="table-action table-action-whatsapp" href={whatsappUrl(client.whatsapp)} target="_blank" rel="noreferrer" aria-label="Abrir WhatsApp"><MessageCircle size={17} /></a><button className="table-action" onClick={() => setCuponFor(client)} aria-label="Dar descuento" title="Dar descuento"><Ticket size={17} /></button><button className="table-action table-action-edit" onClick={() => openEdit(client)} aria-label="Editar"><Edit3 size={17} /></button><button className="table-action table-action-danger" onClick={() => void remove(client)} aria-label="Eliminar"><Trash2 size={17} /></button></div></td></tr>)}</tbody></table></div>
    </>}
    ...
    <CuponModal open={!!cuponFor} cupon={null} clientePreset={cuponFor} onClose={() => setCuponFor(null)} onSaved={() => { setCuponFor(null); toast.success('Cupón creado. Envíaselo al cliente por WhatsApp.') }} />
  </div>
}

export function ClienteModal({ open, cliente, onClose, onSaved }: { open: boolean; cliente: Cliente | null; onClose: () => void; onSaved: (client: Cliente) => void }) {
...
function ClientCard({ client, onEdit, onDelete, onCupon }: { client: Cliente; onEdit: () => void; onDelete: () => void; onCupon: () => void }) { return <article className="rounded-2xl border border-line bg-panel p-4"><div className="flex items-start justify-between"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-accent/10 font-semibold text-accent">{client.nombre.charAt(0)}</span><div><Link to={`/clientes/${client.id}`} className="text-sm font-semibold transition hover:text-accent">{client.nombre}</Link><p className="text-xs text-muted">{client.whatsapp}</p></div></div><button className="table-action table-action-edit" onClick={onEdit} aria-label="Editar"><Edit3 size={17} /></button></div><div className="mt-4 space-y-2 text-xs text-muted"><p className="flex gap-2"><MapPin size={15} />{[client.ciudad, client.departamento].filter(Boolean).join(', ') || 'Sin ubicación'}</p></div><div className="mt-4 flex gap-2"><a className="subtle-button flex-1 text-[#62eaa0]" href={whatsappUrl(client.whatsapp)} target="_blank" rel="noreferrer"><MessageCircle size={16} /> WhatsApp</a><button className="subtle-button" onClick={onCupon} aria-label="Dar descuento" title="Dar descuento"><Ticket size={16} /></button><button className="subtle-button" onClick={onDelete}><Trash2 size={16} /></button></div></article> }
