  ...
  const submit = async (values: FormValues) => {
    try {
      ...
      setFactura({
        ...
        items: normalizedItems.map((item) => ({ producto: item.producto, detalle: [item.marca, item.talla, item.color].filter(Boolean).join(' · ') || undefined, cantidad: 1, precio: Number(item.precio_unitario || 0) })),
        total: ventaTotal,
