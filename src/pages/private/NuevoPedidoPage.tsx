...
const schema = z.object({ cliente_id: z.string().uuid('Selecciona un cliente.'), estado: z.enum(['pedido_confirmado','en_preparacion','control_calidad','etiqueta_creada','despachado','transito_internacional','recibido_estados_unidos','transito_nicaragua','llego_nicaragua','disponible_entrega','pagado','entregado','cancelado','incidencia']), fecha_pedido: z.string().min(1), fecha_estimada: z.string(), abono: z.number().min(0), metodo_pago: z.string(), notas_internas: z.string(), notas_publicas: z.string(), envio_rapido: z.boolean(), items: z.array(itemSchema).min(1) })
...
export function NuevoPedidoPage() {
  ...
  const submit = async (values: FormValues) => {
    try {
      const normalizedItems = values.items.map((item) => ({ ...item, producto: item.producto || item.codigo_producto, cantidad: 1, color: (item.color ?? '').trim(), envio_internacional: 0, costo_delivery: 0, otros_gastos: 0, notas: '', imagen: productos.find((product) => product.id === item.producto_id)?.imagen ?? null }))
      if (!isSupabaseConfigured) { toast.success('Pedido validado en la vista previa.'); navigate('/pedidos'); return }
      if (costo > 0 && !cuentaProveedor.cuentaId) return toast.error('Elegí de qué cuenta pagaste al proveedor.')
      ...
      const created = await crearPedido({ cliente_id: values.cliente_id, estado: values.estado as EstadoPedido, fecha_pedido: values.fecha_pedido, fecha_estimada: values.fecha_estimada || null, abono: values.abono, metodo_pago: values.metodo_pago || null, notas_internas: values.notas_internas || null, notas_publicas: values.notas_publicas || null, envio_rapido: values.envio_rapido, descuento, cupon_id: cuponAplicado?.id ?? null, cupon_codigo: cuponAplicado?.codigo ?? null, items: normalizedItems }, {
        proveedor: costo > 0 ? cuentaProveedor : undefined,
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <form onSubmit={handleSubmit(submit)} className="mt-7 space-y-5">
      ...
      <section className="form-section"><SectionTitle number="03" title="Resumen de venta" /><div className="form-grid mt-5"><Field label="Notas internas"><textarea rows={4} {...register('notas_internas')} /></Field><Field label="Notas visibles para el cliente"><textarea rows={4} {...register('notas_publicas')} /></Field></div>
        ...
        <div className="mt-3 grid gap-3 rounded-xl border border-line bg-white/[0.025] p-4 sm:grid-cols-3 lg:grid-cols-6"><Money label="Venta" value={total} /><Money label="Descuento" value={descuento} /><Money label="Costo" value={costo} /><Money label="Ganancia" value={totalConDescuento - costo} accent /><Money label="Abono" value={abono} /><Money label="Saldo" value={totalConDescuento - abono} /></div><div className="mt-3 grid gap-3 rounded-xl border border-line bg-white/[.02] p-4">{costo > 0 && <CuentaSelect requerido proposito="comprar" montoUsd={costo} tipoCambio={tipoCambio} value={cuentaProveedor} onChange={setCuentaProveedor} modo="resta" label="¿De qué cuenta pagaste al proveedor?" />}<IngresoEnCuenta tipoCambio={tipoCambio} value={ingresoAbono} onChange={setIngresoAbono} label="Abono inicial (dejá el monto en 0 si aún no ha pagado)" /></div>{envioRapido && <p className="mt-2 text-[11px] text-muted">La venta incluye US$15 de envío rápido (14–17 días).</p>}</section>
